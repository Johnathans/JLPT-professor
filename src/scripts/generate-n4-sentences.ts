...
async function main() {
  try {
    console.log('Starting N4 sentence generation...\n');
    ...
    const progressPath = path.join(process.cwd(), 'src', 'data', 'jlpt', 'n4', 'sentences', 'index.ts');
    let currentBatch = 0;
    ...
    while (success && !isShuttingDown) {
      success = await generateSentences('N4', currentBatch);
      if (success) {
        currentBatch++;
    ...
    if (isShuttingDown) {
      ...
      console.log('\nCompleted all N4 sentence generation!');
    }
