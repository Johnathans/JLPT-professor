...
async function generateSentences(level: string, batchNumber: number): Promise<boolean> {
  ...
  try {
    ...
    const response = await fetchWithTimeout(
      ...
      120000 // 2 minute timeout for OpenAI request
    );
...
async function main() {
  try {
    console.log('Starting N5 sentence generation...\n');
    ...
    const progressPath = path.join(process.cwd(), 'src', 'data', 'jlpt', 'n5', 'sentences', 'index.ts');
    let currentBatch = 0;
    ...
    while (success && !isShuttingDown) {
      success = await generateSentences('N5', currentBatch);
      if (success) {
        currentBatch++;
    ...
    if (isShuttingDown) {
      ...
      console.log('\nCompleted all N5 sentence generation!');
    }
